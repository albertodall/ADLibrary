...
namespace AD.ORM.NHibernate
{
    ...
    public class EncryptedString : IUserType, IParameterizedType
    {
        ...
        public void NullSafeSet(IDbCommand cmd, object value, int index)
        {
            ...
            string hashedPassword = _encryptor.Encrypt((string)value);
        ...
        public void SetParameterValues(IDictionary<string, string> parameters)
        {
            if (parameters != null)
            {
                /* Permette di fare una cosa del tipo:
                 * <property name="password">
                 *      <type name="EncryptedString">
                 *          <param name="EncryptionService">AD.Shared.Cryptography.DefaultEncryptionService</param>
                 *      </type>
                 * </property>
                 */
                object parmValue = parameters["encryptionService"];
                _encryptor = parmValue != null ? 
                    (IEncryptionService)Activator.CreateInstance(Type.GetType(parmValue.ToString()))
                    : new DefaultEncryptionService(); 

                parmValue = parameters["encryptionKey"]; 
                if (parmValue != null)
                {
                    _encryptor.EncryptionKey = parameters["encryptionKey"];
                } 
            }
            else
            {
                _encryptor = new DefaultEncryptionService();
            }
        }
